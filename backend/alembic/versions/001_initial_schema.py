...
def upgrade() -> None:
    ...
    # Enum types
    # ------------------------------------------------------------------
    # Enum types — create_type=True so _on_table_create fires on first use.
    # The migration context is configured with checkfirst=True (see env.py) so
    # duplicate-type errors are silently skipped on re-runs.
